	...
	\begin{definition}\label{dfn:christoffel-levi-civita-covariant}
		...
		.\]
		Define the \emph{Levi-Civita connection} $\nabla : \mathfrak{X} (M)\times \mathfrak{X}(M) \to \mathfrak{X} (M)$ on $(M,g)$ as the map
		\[
			\nabla (\partial_{x^i} ,\partial_{x^j})=\sum_{k=1} ^n \Gamma_{ij} ^k \partial_{x^k}
		\]
		which is $C^\infty (M)$-linear (and thus $\mathbb{R} $-linear via constant functions) in the first argument and satisfies the following product rule in the second: for any smooth $f: M \to \mathbb{R} $,
		\[
			\nabla_X(fY)=f\nabla _XY+(Xf)Y
		.\]
		We generally write $\nabla(X,Y)$ as $\nabla _XY$. We will often call $\nabla _XY$ the \emph{covariant derivative} of $Y$ in the direction of $X$.
	\end{definition}
	\begin{remark}
