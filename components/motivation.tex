	...
	\begin{example}\label{ex:punctured-plane-computation}
		...
		semicircular arc has length $\pi \varepsilon$. Writing $L_g$ for the length of a curve $\gamma _\varepsilon $,
		\[
		L_g(\gamma_\varepsilon)
		= 2(1-\varepsilon) + \pi\varepsilon
		= 2 + (\pi - 2)\varepsilon.
		\]
		As $\varepsilon \to 0^+$ we get $L_g(\gamma_\varepsilon)\to 2$. If we view distance $d_g$ as the infimum over lengths of piecewise smooth curves from $p$ to $q$, then $d_g(p,q)=2$.
		...
		segment from $(1,0)$ to $(-1,0)$, and that passes through the origin.  Thus $d_g(p,q)=2$, but there is no curve $\gamma$ in $M$ with $L_g(\gamma) = d_g(p,q)$.
	\end{example}
	...
	completeness in the study of geodesics. In a \emph{complete} or \emph{geodesically complete} and connected Riemannian manifold, any two points can be joined by a geodesic of minimal length (\cite[Ch9~Thm.~18]{spivak}).
